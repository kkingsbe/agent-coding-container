...
async function runKiloWithPrompt(promptContent, promptName) {
  logWithTimestamp(`Starting ${promptName}...`);
  logWithTimestamp(`Target Workspace: ${WORKSPACE_PATH}`);

  const executor = new CLIExecutor();

  logWithTimestamp(`Running 'kilocode' with piped ${promptName} (${promptContent.length} chars)...`);
  ...
  const result = await executor.execute({
    ...
    onErrorDetected: (pattern) => {
      logWithTimestamp(`Error pattern detected: ${pattern}`);
    }
  });

  if (result.terminatedEarly) {
    logWithTimestamp(`${promptName} terminated early: ${result.terminationReason} (${result.executionTimeMs}ms)`);
  } else if (result.exitCode === 0) {
    logWithTimestamp(`${promptName} completed successfully (${result.executionTimeMs}ms)`);
  } else {
    logWithTimestamp(`${promptName} completed with exit code ${result.exitCode} (${result.executionTimeMs}ms)`);
  }
...
Examples:
  node run-janitor.js
  node run-janitor.js --workspace /my/workspace --interval 1800000
  node run-janitor.js --state /my/state --no-immediate
  node run-janitor.js --timeout 600000
`);
}

// Set up environment
const TASK_NAME = 'janitor';
...
/**
 * Generate output file path for janitor delegation instructions
 * @returns {string} Full path to output file
 */
function getOutputFilePath() {
  const timestamp = Date.now();
  return path.join(WORKSPACE_PATH, `.janitor-output-${timestamp}.md`);
}
...
/**
 * Check for continuation marker file
 * @returns {string|null} Name of the prompt in progress, or null
 */
function checkContinuation() {
    const markerPath = path.join(WORKSPACE_PATH, '.janitor_in_progress');
    if (fsSync.existsSync(markerPath)) {
        const stateFilePath = path.join(WORKSPACE_PATH, 'JANITOR_STATE.md');
        const hasStateFile = fsSync.existsSync(stateFilePath);
        console.log('🔄 Found incomplete janitor session');
        console.log(`   - Marker: .janitor_in_progress`);
...
/**
 * Main janitor execution handler
 * This function is called by the scheduler on a recurring basis
 */
async function janitorExecutionHandler() {
  const handlerTimestamp = new Date().toISOString();
  logWithTimestamp(`[${TASK_NAME}] Starting janitor execution handler at ${handlerTimestamp}`);
  ...
  try {
    ...
    const [todoContent, backlogContent, completedContent, blockersContent] = await Promise.all([
    ...
    logWithTimestamp(`[${TASK_NAME}] COMPLETED.md length: ${completedContent.length} bytes`);
    logWithTimestamp(`[${TASK_NAME}] BLOCKERS.md length: ${blockersContent.length} bytes`);
    
    // Build context for prompt execution
    const context = {
      taskName: TASK_NAME,
      timestamp: handlerTimestamp,
      todoContent: todoContent || '(empty)',
      backlogContent: backlogContent || '(empty)',
      completedContent: completedContent || '(empty)',
      blockersContent: blockersContent || '(empty)'
    };
    
    // Execute the JANITOR.md template as an orchestrator
    logWithTimestamp(`[${TASK_NAME}] Executing JANITOR.md template as orchestrator...`);
    const promptResult = await executePromptTemplate('JANITOR.md', context);

    if (!promptResult.success) {
      throw new Error(`Failed to execute JANITOR.md template: ${promptResult.error}`);
    }

    // Execute the Kilo Code CLI with the processed prompt
    logWithTimestamp(`[${TASK_NAME}] Executing Kilo Code CLI with JANITOR prompt...`);
    const result = await runKiloWithPrompt(promptResult.output, 'JANITOR');
    ...
    // Write output to the workspace directory for reference
    const outputFile = getOutputFilePath();
    await fs.writeFile(outputFile, promptResult.output, 'utf-8');
    logWithTimestamp(`[${TASK_NAME}] Janitor delegation instructions written to ${outputFile}`);

    // Update state with output file
    await stateManager.updateState(TASK_NAME, {
      lastOutputFile: outputFile
    });
    logWithTimestamp(`[${TASK_NAME}] State updated with output file`);
    ...
    logWithTimestamp(`[${TASK_NAME}] Janitor execution handler completed successfully`);
    
  } catch (error) {
    logWithTimestamp(`[${TASK_NAME}] Error in janitor execution handler: ${error.message}`);
...
const task = createScheduledTask(janitorExecutionHandler, {
  taskName: TASK_NAME,
...
logWithTimestamp(`[${TASK_NAME}] Starting Janitor Runner`);
logWithTimestamp(`[${TASK_NAME}] Workspace Path: ${WORKSPACE_PATH}`);
logWithTimestamp(`[${TASK_NAME}] State Path: ${STATE_PATH}`);
logWithTimestamp(`[${TASK_NAME}] Schedule Interval: ${SCHEDULE_INTERVAL_MS}ms (20 minutes)`);
